...
<script>
  ...
  function submitForm(action, url) {
    ...
    if (form.valid()) {
      ...
      alert("Please fill up the input properly.")
      console.log("test");
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $(data).each(function (index, file) {
          ...
          $('<td>').text(new Date(file.createdAt).toLocaleString()).appendTo(fileRow);
          $('<td>').text(file.size/1000 + " KB").appendTo(fileRow);
  ...
  function editEmployee(id, firstName, lastName, email, birthDate) {
    ...
    $('#email').val(email);
    $('#birthDate').val(new Date(new Date(birthDate).getTime() - (new Date("May 09, 2023").getTimezoneOffset() * 60000)).toISOString().split('T')[0]);
